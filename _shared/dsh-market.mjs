 ...
 *
 * 用法（在 pwsh/bash 中）：
 *   node "$HOME/.config/mimocode/skills/_shared/dsh-market.mjs" index [secids]        # 指数/多标的多字段
 *   node "$HOME/.config/mimocode/skills/_shared/dsh-market.mjs" stocks "1.688017,0.300718"   # 个股（东财）
 *   node "$HOME/.config/mimocode/skills/_shared/dsh-market.mjs" sector [pz]          # 板块资金流向（东财）
 *   node "$HOME/.config/mimocode/skills/_shared/dsh-market.mjs" sina "sh600519,sz000001"     # 新浪实时（GBK）
 *   node "$HOME/.config/mimocode/skills/_shared/dsh-market.mjs" kline "SH600519" [period=101] [limit=120] [fqt=1]
 *
