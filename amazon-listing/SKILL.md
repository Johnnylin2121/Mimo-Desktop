...
description: >
  ...
  combined ≤200). Supports any Amazon marketplace. Crawling via webfetch,
  playwright skill (fallback for truncation/CAPTCHA/login-state), or
...
Output: single progressively-built `.md` file written to `{VAULT_PATH}/工作/亚马逊工作管理/链接listing/<ASIN>-<product>-listing.md` (DSH: read the vault path from `MEMORY.md（本机记忆）` — never hardcode it). **All Amazon-related outputs must live under `{VAULT_PATH}/工作/亚马逊工作管理/`** — never create top-level `{VAULT_PATH}/工作/亚马逊分析/` or similar new folders.
...
5. **Organic ranking gap re-check (自然位缺口回检, added 2026-09-17)** — cross-skill step with `amazon-ad-analysis` Phase 5.4-B:
   ...
   - **Measured path**: with the logged-in Amazon product page open (playwright skill), open the **Sorftime** plugin panel's 「反查关键词」. Plugin identity: `Sorftime Save`, extension id `aadiiicebnjmjmibjengdohedcfeekeg` (v1.4.8.7). Verified on amazon.com.mx with a logged-in account. NOT Sellersprite (卖家精灵) — the two are similar but distinct. **If browser/extension unavailable → record "B级数据未取到，自然位未验证" and do NOT substitute a guess.**
