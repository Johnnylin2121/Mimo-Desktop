...
本环境**无** `obsidian_*` 插件工具（DSH 的 dsh-obsidian 不适用）。Vault 读写一律用内置文件工具 + `_shared/vault-batch.mjs`：

| 操作 | 工具 |
|------|------|
| 全文检索/定位笔记 | `grep` / `glob` |
| 读取笔记（正文+frontmatter） | `read` |
...
| 标签汇总 / 库结构 | `node .../vault-batch.mjs structure\|orphans\|dangling` |
| frontmatter 读写 | `read` + `edit` 手工编辑 YAML |

**规则**：
- 路径参数一律相对 vault 根（如 `wiki/entities/证监会.md`），不越界写 `{VAULT_PATH}` 之外。
- 删除一律进 `.trash/`，绝不永久删除——符合"历史文件必须保留"规则。
- OneDrive 批量写入仍须分批，避免触发同步锁。
- 一次要动整目录或一次多篇：用 `_shared/vault-batch.mjs`。该脚本保留 `|别名`/`#锚点`/`![[嵌入]]`，路径式链接仍写路径、裸名仍写裸名，并清理空目录；**先加 `--dry-run` 看改写清单再执行**。库结构/孤立笔记/悬空链接体检：`node "$HOME/.config/mimocode/skills/_shared/vault-batch.mjs" structure|orphans|dangling`。
...
> **文中间编辑写法**：无逐字编辑工具时，在「近期动态」等已有节内插入内容、更新既有计数——`read` 读取后 `edit` 局部改写，或 `write` 全量重写；文末追加用 `edit`。frontmatter 字段直接改 YAML 头。
